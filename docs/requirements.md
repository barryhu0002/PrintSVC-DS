...
1. 启动东芝 E-Studio 240s 老式打印机和 PrintSVC 程序，启动顺序不分先后。PrintSVC 启动后默认以系统托盘程序形式运行，不显示 GUI 窗口。
...
- 程序启动后默认不显示 GUI 窗口，作为 Win7 系统托盘程序运行在后台。
- 托盘图标右键菜单提供以下功能：
  - **打开窗口**：打开本地 Web 状态页，显示打印机状态、服务状态和作业信息。
  - **清空日志**：清空当前日志文件，程序继续运行。
  - **关闭程序**：干净停止 IPP 服务、局域网发现服务并退出程序。
- 托盘模式必须兼容 PyInstaller 打包后的单文件 EXE 运行方式。
- 默认启动方式不再依赖用户手动打开主窗口；浏览器页面作为主要状态查看界面。
